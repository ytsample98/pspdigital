...
// Run every minute
cron.schedule(`* * * * *`, async () => {
    // console.log("Running PSC Scheduler...");

    try {
        const escalationLevels = await db.query(`SELECT * FROM escalation`);
        const escRecords = escalationLevels.rows;
        if (!escRecords.length) return;

        const levelDurations = {
            1: escRecords.find(e => e.authority_id === 2)?.time_duration || 0,
            2: escRecords.find(e => e.authority_id === 3)?.time_duration || 0,
            3: escRecords.find(e => e.authority_id === 4)?.time_duration || 0,
        };

        // console.log("Level durations:", levelDurations);

        // LEVEL 1 → LEVEL 2
        const level1Records = (await db.query(`
            SELECT psccard_id, escalation_id, status, due_time
            FROM escalation_level1
            WHERE due_time >= NOW() - INTERVAL '${levelDurations[2]} DAY'
              AND due_time < NOW() - INTERVAL '${levelDurations[1]} DAY'
              AND status='Open' AND user_resp_id=2 AND user_rep_level=0
        `)).rows;

        // console.log(`Found ${level1Records.length} Level1 records to process.`);

        await Promise.all(level1Records.map(async record => {
            console.log(`Processing Level1 record: psccard_id=${record.psccard_id}`);
            const exists = parseInt((await db.query(
                "SELECT COUNT(*) AS count FROM escalation_level2 WHERE psccard_id = $1",
                [record.psccard_id]
            )).rows[0].count, 10);

            console.log(`Existence check for escalation_level2 record with psccard_id=${record.psccard_id}: Count = ${exists}`);
            if (!exists) {
                await db.query(
                    "INSERT INTO escalation_level2 (psccard_id, escalation_id, user_resp_id, status, timestamp_escalated, due_time) VALUES ($1, $2, $3, $4, NOW(), $5)",
                    [record.psccard_id, record.escalation_id, 3, record.status, record.due_time]
                );
                ...
                console.log(`Moved Level1 → Level2: psccard_id=${record.psccard_id}`);
            }
        }));

        // LEVEL 2 → LEVEL 3
        const level2Records = (await db.query(`
            SELECT psccard_id, escalation_id, status, timestamp_escalated
            FROM escalation_level2
            WHERE timestamp_escalated < NOW() - INTERVAL '${levelDurations[2]} DAY'
              AND status='Open' AND user_resp_id=3 AND user_rep_level=0
        `)).rows;
        // console.log(`Found ${level2Records.length} Level2 records to process.`);
        await Promise.all(level2Records.map(async record => {
            console.log(`Processing Level2 record: psccard_id=${record.psccard_id}`);
            const exists = parseInt((await db.query(
                "SELECT COUNT(*) AS count FROM escalation_level3 WHERE psccard_id = $1",
                [record.psccard_id]
            )).rows[0].count, 10);

            console.log(`Existence check for escalation_level3 record with psccard_id=${record.psccard_id}: Count = ${exists}`);
            if (!exists) {
                await db.query(
                    "INSERT INTO escalation_level3 (psccard_id, escalation_id, user_resp_id, status, timestamp_escalated, due_time) VALUES ($1, $2, $3, $4, NOW(), $5)",
                    [record.psccard_id, record.escalation_id, 4, record.status, record.timestamp_escalated]
                );
                ...
                console.log(`Moved Level2 → Level3: psccard_id=${record.psccard_id}`);
            }
        }));

        if (level1Records.length || level2Records.length) {
            console.log("Scheduler cycle completed.");
        }

    } catch (error) {
        console.error("Error running PSC Scheduler:", error);
    }
});