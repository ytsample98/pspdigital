...
class contractapproval extends Component {
  ...
  fetchAwaiting = async () => {
    ...
    // filter to statuses that need approval (same logic as BillingApproval)
    const awaiting = data.filter(c => c.status === 'Awaiting Approval' || c.status === 'Entered');
    this.setState({ contracts: awaiting });
  };
          ...
          <table className="table table-bordered table-hover">
            ...
            <tbody>
              {this.state.contracts.map(c => {
                ...
                if (c.status === "Entered" || c.status === "Awaiting Approval") statusClass = "badge-warning";
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td>{c.customer}</td>
                    <td>{c.createdDate || c.contrDurationFrom}</td>
              ...
              {this.state.contracts.length === 0 && (
                <tr><td colSpan="6" className="text-center">No contracts awaiting approval</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    return (
      <div className="card mt-4 p-4 shadow-sm full-height d-flex flex-column">
        ...
        <div className="row mb-3">
          ...
          <div className="col-md-4"><b>Created:</b> {c.createdDate}</div>
          <div className="col-md-4"><b>Status:</b> {c.status}</div>
        </div>
        <div className="row mb-3">
          <div className="col-md-4"><b>Std Days:</b> {c.stdDays}</div>
          <div className="col-md-4"><b>Value:</b> {c.contractValue || c.amtAgreed}</div>
