...
export class Dashboard extends Component {
  ...
  fetchLatestOrderAlert = async () => {
  const snap = await getDocs(collection(db, 'orders'));
  const orders = snap.docs.map(doc => doc.data());
  if (!orders.length) return;

  const latestOrder = orders[orders.length - 1];
  const breakdownItems = latestOrder.breakdownItems || [];

  if (breakdownItems.length > 0) {
    const lastItem = breakdownItems[breakdownItems.length - 1];
    this.setState({
      showAlert: true,
      alertMessage: lastItem.description || 'No Description',
      alertDate: lastItem.date || ''
    });
  }
};
  ...
  <Modal.Header closeButton>
    <Modal.Title>Reminder: Breakdown Due</Modal.Title>
  </Modal.Header>
  <Modal.Body>
    <p><strong>Date:</strong> {this.state.alertDate}</p>
