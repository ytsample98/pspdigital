...
const handleActionToggle = (id, isChecked) => {
  setUiState(prev => ({
    ...prev,
    [id]: {
      ...(prev[id] || {}),
    ...
    countermeasures.filter(cm => cm.id).map((cm, idx) => (
      <tr key={cm.id || idx + 1}>
        ...
        <td>
          <input
            type="checkbox"
            checked={uiState[cm.id|| idx + 1]?.actionTaken || false}
            onChange={(e) => handleActionToggle(cm.id|| idx + 1, e.target.checked)}
          />
        </td>
        <td>
          {uiState[cm.id|| idx + 1]?.showRemarks ? (
            <input
              type="text"
              className="form-control"
               value={uiState[cm.id|| idx + 1]?.remarks || ''}
              onChange={(e) => handleRemarksChange(cm.id|| idx + 1, e.target.value)}
